...
def dataset_inference(model, device: str, image_input_dir: Path, label_input_dir:Path , output_dir: Path, class_list:list, detection_threshold:float, resize: tuple):
    '''
    ...
    for i in range(len(test_images)):
        ...
        pred_classes = [class_list[j] for j in outputs[0]['labels'].cpu().numpy()]

        writer = Writer(str(output_dir.joinpath("images", f"{image_name}.jpg")),orig_width , orig_height)
        # draw the bounding boxes and write the class name on top of it
        for j, box in enumerate(draw_boxes):
            class_name = pred_classes[j]
            color = COLORS[class_list.index(class_name)]
            orig_image = custom_utils.draw_boxes(orig_image, box, color, resize)
            ...
            #plt.imshow('Prediction', orig_image)
            #cv2.waitKey(1)
