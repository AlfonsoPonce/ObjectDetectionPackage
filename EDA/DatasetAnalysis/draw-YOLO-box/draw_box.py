...
def plot_one_box(x, image, color=None, label=None, line_thickness=None):
    ...
    if True:
    #if 0 < bbox_area and bbox_area < 1024:
        ...
        if label:
            ...
            cv2.putText(image, label+str(bbox_area), (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        return True
    else:
        return False
...
def draw_box_on_image(image_name, classes, colors, label_folder, raw_images_folder, save_images_folder ):
    ...
    for line in source_file: #例遍 txt文件得每一行
        ...
        is_small = plot_one_box([x1,y1,x2,y2], image, color=colors[class_idx], label=classes[class_idx], line_thickness=None)
        #if is_small:
